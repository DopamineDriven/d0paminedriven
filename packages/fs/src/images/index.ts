...
import type { ImageSpecs, ExpandedImgSpecs } from "@/types/index.ts";
...
export class ImgMetadataExtractor extends ImgMetadataExtractorWorkup {
  ...
  public tiff(
    ...
    public async extractImgSpecsViaStream(stream: Readable, size = 4096) {
    ...
    public async extractImgSpecsViaPath(filePath: string, size = 4096): Promise<ExpandedImgSpecs> {
      const stream = createReadStream(relative(process.cwd(), filePath), {
        highWaterMark: size
      });
      return await this.extractImgSpecsViaStream(stream, size);
    }
